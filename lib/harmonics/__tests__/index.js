...
describe('harmonics', function () {
  test('it checks constituents', function () {
    ...
    try {
      testHarmonics = new _index["default"]('not array');
    } catch (error) {
      errorMessage = error;
    }
    ...
    try {
      testHarmonics = new _index["default"]([{
        name: 'M2',
    ...
    expect(errorMessage.message).toBe('Harmonic constituent entry missing field speed');
  });
